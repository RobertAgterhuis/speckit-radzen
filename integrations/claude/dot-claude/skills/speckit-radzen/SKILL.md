---
name: speckit-radzen
description: Repository-aware, MCP-first feature engineering for Radzen Blazor. Use when adding or changing Radzen Blazor UI (grids, forms, dialogs, dashboards) or any feature that needs Radzen UI changes: detects the project, verifies Radzen APIs through the Radzen MCP, and enforces quality gates G0-G8.
---
...
`.speckit/radzen` is the installed kit folder. `speckit-radzen` means `pwsh .speckit/radzen/tools/speckit-radzen.ps1` (add `-Json` for machine-readable output).
...
Read only what the current phase needs: the workflow file `.speckit/radzen/core/workflows/NN-*.md`, plus the files it lists. Always keep the constitution (`.speckit/radzen/core/constitution/constitution.md`) and local amendments (`.speckit/radzen/local/`) in force.
...
3. Otherwise query MCP with the exact component name, data shape, interaction and version (`.speckit/radzen/core/mcp/query-playbook.md`).
...
6. Unavailable MCP, quota or conflicts: `.speckit/radzen/core/mcp/fallback-matrix.md`.
...
- Constitution: `.speckit/radzen/core/constitution/constitution.md`
- Workflows: `.speckit/radzen/core/workflows/README.md`
- Gates: `.speckit/radzen/core/gates/quality-gates.md`
- Anti-patterns: `.speckit/radzen/core/antipatterns/index.md`
- Standards: `.speckit/radzen/core/standards/README.md` · Patterns: `.speckit/radzen/core/patterns/README.md`
- MCP: `.speckit/radzen/core/mcp/mcp-workflow.md`
- Roles: `.speckit/radzen/core/agents/roles/`
